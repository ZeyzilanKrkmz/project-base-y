...
router.post("/update",async(req,res)=>{
  ...
  try{
    ...
    if(typeof body.is_active==="boolean") updates.is_active=body.first_name;
    ...
    if(Array.isArray(body.roles)&&body.roles.length>0){
      ...
      if(removedRoles.length>0){
        await UserRoles.deleteMany({_id:{$in: removedRoles.map(r=>r.role_id)}})
      }
...
router.post("/delete",async(req,res)=>{
  try{
    ...
    await Users.deleteMany({user_id:body._id});
